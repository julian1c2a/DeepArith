...
### (3.) Espacios de nombres anidados (Nested Namespaces)

La arquitectura de este proyecto utiliza un sistema de namespaces anidados estricto para separar conceptos:
- `PROJECT_NAME.Syntax`: Contiene las teorías puramente formales y sintácticas (en lógica de primer orden).
- `PROJECT_NAME.Semantics`: Contiene los modelos y estructuras que dan semántica a la sintaxis.
- `PROJECT_NAME.Foundation`: Contiene los axiomas opacos y pruebas de isomorfismos/unicidad fundacionales.
- Todo desarrollo posterior debe encapsularse en sub-namespaces (ej. `PROJECT_NAME.Syntax.Peano`).
- Los namespaces no son necesariamente iguales a los módulos (archivos). Documentar en el sistema REFERENCE qué namespaces existen, a qué módulos pertenecen y cómo se relacionan entre sí. Todos los namespaces deben derivar de `PROJECT_NAME`.
...
Todos los módulos de trabajo deben estar contenidos estrictamente dentro del directorio del proyecto (ej. `DeepArithmetic/`) o en subdirectorios temáticos dentro de él (ej. `DeepArithmetic/Theorems/`). 
El único archivo `.lean` permitido en la raíz es el módulo raíz autogenerado (`DeepArithmetic.lean` o el nombre que reciba en `gen-root.bash`).

### (17.) Bloques de Exportación (*Export blocks*)

Todo módulo de producción (hoja) **DEBE** terminar con una lista de exportación real y sintácticamente válida (no en un comentario). Esta instrucción `export` lista todas las definiciones, teoremas y lemas públicos creados en ese módulo, y **tiene que aparecer inmediatamente después de cerrar el namespace**. Las declaraciones `private` (o con sufijo `_aux`) nunca se exportan.

```lean
end PROJECT_NAME.SubModulo

-- Lista de exportación real inmediatamente después
